# Travel Agency Operations Analysis (Excel End-to-End Project)
## Project Overview
This project explores travel agency performance using operational data from August 2024 to July 2025. The goal was to uncover trends in bookings, revenue, customer satisfaction, and regional demand to support smarter business decisions.

From raw data chaos to a structured, interactive dashboard, this project demonstrates the full data analysis lifecycle using Microsoft Excel.
## Key Objectives
- Identify booking trends across months
- Evaluate agent performance based on revenue
- Understand customer preferences by trip type
- Highlight high-demand regions
- Enable dynamic filtering using slicers
## Tools & Skills Used
- Microsoft Excel
- Data Cleaning & Transformation
- Pivot Tables
- Data Visualization
- Dashboard Design
- Analytical Thinking
## Data Cleaning & Preparation
- Handled missing and inconsistent values
- Standardized date formats for time-based analysis
- Cleaned categorical fields (trip types, regions, agent names)
- Created calculated fields for deeper insights
## Dashboard Highlights
📅 Booking Trends by Month
* Peak bookings: August 2024 (55 bookings)
* Lowest: July 2024 (12 bookings)
* Recovery observed: July 2025 improved to 28 bookings
  
🌍 Revenue by Agent
* Top performer: Karen Sutton – $58,214
* Lowest performer: Deborah Morgan
  
⭐ Average Rating by Trip Type
* Highest rated: City Break – 4.14/5
  
📍 Top 10 Regions by Bookings
* Leading destination: Louisiana – 51 bookings

## Interactive Features
* Slicers for Loyalty Tier and Season
* Enables dynamic filtering for targeted insights

## Key Insights
* Seasonal trends significantly impact booking volumes
* Certain agents consistently outperform others, indicating opportunities for training or strategy alignment
* Customer satisfaction is highest for shorter, flexible trips (City Breaks)
* Regional demand is concentrated, suggesting targeted marketing opportunities

## Business Impact
This dashboard provides decision-makers with:

* Clear visibility into performance trends
* Data-driven insights for revenue optimization
* A foundation for improving customer experience
* Strategic direction for regional marketing

## Files Included
- <a href="https://github.com/mugabepatricia/Travel-Agency-Operations-Analysis-Excel-End-to-End-Project-/blob/main/Travel%20Agency%20Operations%20Analysis.xlsx">Travel Agency Dataset</a>
- <a href="https://github.com/mugabepatricia/Travel-Agency-Operations-Analysis-Excel-End-to-End-Project-/blob/main/Screenshot%202026-04-10%20at%2011.55.48.png">Dashboard Preview</a>

## How to Use
1. Download the Excel file
2. Open in Microsoft Excel
3. Use slicers to interact with the dashboard

## About Me
Aspiring Data Analyst passionate about transforming raw data into actionable insights. Skilled in Excel, data storytelling, and dashboard creation.
